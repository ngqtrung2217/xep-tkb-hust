...
export default function Home() {
  ...
  const groupedDetail = currentResult ? (() => {
    const g = new Map<string, ClassSession[]>()
    for (const s of currentResult.sessions) {
      const key = s.maLopKem && s.maLopKem !== 'NULL' && s.maLopKem !== s.maLop ? s.maLopKem : s.maLop
      if (!g.has(key)) g.set(key, [])
      g.get(key)!.push(s)
    }
    return [...g.values()].sort((a, b) => a[0].day * 100 + a[0].startPeriod - (b[0].day * 100 + b[0].startPeriod))
  })() : []
            ...
            <div className="px-4 py-3 border-b">
              ...
              <textarea
                ...
                onPaste={e => {
                  ...
                  const codes = (text.match(/[A-Za-z]{2}\d{4}[A-Za-z]?/g) || [])
                    .map(c => c.toUpperCase())
            ...
            {currentResult && <div className="mt-6 bg-white rounded-xl border p-4 shadow-sm">
              ...
              {scheduleResults && <div className="mt-3">
                <div className="space-y-1.5">
                  {groupedDetail.map((group, i) => {
                    const s = group[0]
                    const color = courseColors.get(s.courseCode) || '#888'
                    const course = data?.courses.get(s.courseCode)
                    const times = group.map(s => `${DAY_LABELS[s.day]} ${s.timeStr}`).join(', ')
                    return (
                      <div key={i} className="text-sm bg-gray-50 rounded-lg px-3 py-2 border flex items-center gap-x-3 gap-y-1 flex-wrap">
                        ...
                        <span className="text-gray-500 flex-shrink-0">{times}</span>
